...
import { useUserConsumed, useUserConsumedData, useUserConsumedTotal } from "@/app/componets/zustand/consumed";
...
import { useUserExpensesCategories, useUserExpensesCategoriesData, useUserExpensesCategoriesTotal } from "@/app/componets/zustand/expenseCategories";
import { useUserExpenses, useUserExpensesData, useUserExpensesTotal } from "@/app/componets/zustand/expenses";
        ...
        try {
            await signOut(auth)
                .then(() => {
                    ...
                    useUserLogsData.setState({ userLogsTotal: null })
                    useUserLogsTotal.setState({ userLogsData: null })
                    ...
                    useUserExpensesCategories.setState({ userOrders: null })
                    ...
                    useUserExpenses.setState({ userExpenses: null })
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="relative">
                                <select
                                    ...
                                    className={`rounded px-3 py-3 w-full text-xs sm:text-base transition-all
      ${theme === "Dark"
                                            ? "bg-gray-300 text-black focus:ring-2 focus:ring-blue-500"
                                            : "border text-black shadow-lg focus:ring-2 focus:ring-blue-400"}
    `}
                                >
                                    <option value="">Select Role</option>
                                    <option value="Admin">Admin</option>
            ...
            {/* Manage Roles  */}
            {roleModalEdit && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80 px-4 py-6">
                    <div
                        className={`p-4 sm:p-6 rounded-2xl shadow-xl w-full max-w-md 
        max-h-[90vh] overflow-y-auto  {/* Enables scrolling on small screens */}
            ...
            `}
                                >
                                    ...
                                    <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
                                        <span className="font-semibold text-xs sm:text-lg">{role}</span>
                                        <span
                ...
                `}
                  ...
                  `}
                        ...
                        <div className="flex justify-end mt-5">
                            <button
                                className={`px-5 py-1.5 rounded-lg font-medium text-xs sm:text-base transition-colors
            ${theme === "Dark"
